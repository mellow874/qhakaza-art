...
/**
 * Wipes the tables Vera owns, in foreign-key-safe order. Called before each
 * integration test so they cannot leak state into one another.
 *
 * Collector tables are deliberately absent: Vera cannot write them, so it has
 * no business clearing them either. Leaving them out keeps the boundary honest
