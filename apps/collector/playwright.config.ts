...
export default defineConfig({
  ...
  webServer: {
    ...
    /*
     * A DB-backed route, deliberately. `/collectors` is statically rendered, so
     * it answers 200 while Prisma is still initialising — and the first test to
     * submit a form then pays the whole cold start inside its own timeout and
     * fails on timing rather than behaviour. `/private/<token>` validates a
     * token, so a 200 here means the database path is genuinely ready.
     */
    url: `${baseURL}/private/readiness-probe`,
