 ...
 *
 * Kept out of server-action files on purpose: a "use server" module may
 * only export async functions, so exporting a schema from it breaks the build.
 *
 * Language: "appreciation period" and "placed with" — avoiding loan, rental,
 * return terminology. Pending Qhakaza confirmation of the collector-facing
 * language.
 */
...
export type CreateCustodyPeriodInput = z.infer<typeof createCustodyPeriodSchema>;
...
export type ExtendCustodyPeriodInput = z.infer<typeof extendCustodyPeriodSchema>;
...
export type ConcludeCustodyPeriodInput = z.infer<typeof concludeCustodyPeriodSchema>;
