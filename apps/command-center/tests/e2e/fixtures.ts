...
export const test = base.extend<{
  ...
  // ADVISOR reaches the console but not the permissions controls, so the two
  // staff roles need separate fixtures to tell those cases apart.
  advisor: async ({}, use) => {
    const account = await createAccount('ADVISOR');
    await use(account);
    await destroyAccount(account);
  },
});
